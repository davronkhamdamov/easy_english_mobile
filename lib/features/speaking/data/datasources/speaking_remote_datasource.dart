...
class SpeakingRemoteDatasource {
  ...
  Future<String> transcribeSpeakingAudio(String audioFilePath) async {
    debugPrint(
      'Sending audio file to Whisper API (https://easy-english.uz/api/v1/ai/transcribe-speaking)...',
    );
    final response = await _client.postMultipart(
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      final json = jsonDecode(response.body) as Map<String, dynamic>;
      final text = (json['transcript'] ?? json['transcription'] ?? '')
          .toString();
      debugPrint('Whisper transcription success: $text');
      return text;
    } else {
      debugPrint(
        'Whisper error response: HTTP ${response.statusCode} - ${response.body}',
      );
      throw Exception(
