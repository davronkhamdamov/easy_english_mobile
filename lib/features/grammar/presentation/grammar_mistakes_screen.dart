...
class GrammarMistakesScreen extends StatefulWidget {
  ...
  const GrammarMistakesScreen({Key? key, this.initialMistakes}) : super(key: key);
...
class _GrammarMistakesScreenState extends State<GrammarMistakesScreen> {
  ...
  List<GrammarMistakeRecord> get _filteredMistakes {
    return _mistakes.where((m) {
      final matchesCategory = _selectedCategory == 'All' || m.topicCategory == _selectedCategory;
      final matchesSearch = _searchQuery.isEmpty ||
          m.originalSentence.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          m.correctedSentence.toLowerCase().contains(_searchQuery.toLowerCase()) ||
  ...
  void _toggleResolve(GrammarMistakeRecord mistake) {
    setState(() {
      ...
      if (idx != -1) {
        _mistakes[idx] = _mistakes[idx].copyWith(isResolved: !_mistakes[idx].isResolved);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Grammar Mistake Tracker', style: TextStyle(fontWeight: FontWeight.bold)),
        elevation: 0,
          ...
          IconButton(
            ...
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Filtering by highest occurrence count.')),
              );
            },
          ),
        ],
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Container(width: 1, height: 40, color: isDark ? AppColors.darkBorder : AppColors.lightBorder),
          _statCol('Resolved', '${_mistakes.length - unresolvedCount}', AppColors.success),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Row(
            children: [
                ...
                child: Row(
                  children: [
                    Icon(
                      ...
                      size: 12,
                      color: mistake.occurrenceCount >= 3 ? AppColors.danger : AppColors.warning,
                    ),
              ...
              IconButton(
                icon: Icon(
                  isResolved ? Icons.check_circle_rounded : Icons.radio_button_unchecked,
            ...
            child: TextButton.icon(
              onPressed: () {
                ...
                final sampleTopic = GrammarSeedData.sampleTopics.firstWhere(
                  (t) => t.title.toLowerCase().contains(mistake.topicCategory.toLowerCase()),
                  orElse: () => GrammarSeedData.sampleTopics.first,
                );
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => GrammarExerciseScreen(topic: sampleTopic),
                  ),
                );
              },
