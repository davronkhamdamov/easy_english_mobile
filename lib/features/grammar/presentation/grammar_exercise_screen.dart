...
class GrammarExerciseScreen extends StatefulWidget {
  ...
  const GrammarExerciseScreen({Key? key, required this.topic}) : super(key: key);
...
class _GrammarExerciseScreenState extends State<GrammarExerciseScreen> {
  ...
  GrammarRule? get _currentRule {
    try {
      return widget.topic.rules.firstWhere((r) => r.id == _currentExercise.ruleId);
    } catch (_) {
  ...
  void _submitAnswer() {
    ...
    final isRight = userAnswer.toLowerCase() == exercise.correctAnswer.toLowerCase();
        ...
        _sessionMistakes.add(
          GrammarMistakeRecord(
            ...
            originalSentence: exercise.sentenceWithBlank != null
                ? exercise.sentenceWithBlank!.replaceAll('_____', '"$userAnswer"')
                : 'User selected: "$userAnswer"',
            correctedSentence: exercise.sentenceWithBlank != null
                ? exercise.sentenceWithBlank!.replaceAll('_____', '"${exercise.correctAnswer}"')
  ...
  double get _calculatedMastery {
    ...
    return (widget.topic.masteryPercentage * 0.4 + newScore * 0.6).clamp(0.0, 100.0);
  }
        ...
        return AlertDialog(
          ...
          title: Column(
            children: [
              ...
              Text(
                isPassed ? 'Topic Mastery Achieved!' : 'Practice Session Complete',
          ...
          content: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    const Text('New Topic Mastery Rating', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                Navigator.pop(context, finalMastery); // Return updated mastery to roadmap
              },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.topic.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        elevation: 0,
                ...
                children: [
                  ...
                  if (exercise.type == ExerciseType.multipleChoice) ...[
                    ...exercise.options.map((opt) {
                      ...
                      Color optionBg = isDark ? AppColors.darkSurfaceVariant : AppColors.lightSurfaceVariant;

                      if (_isAnswerSubmitted) {
                        if (opt.toLowerCase() == exercise.correctAnswer.toLowerCase()) {
                        ...
                        child: InkWell(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: optionBg,
                              borderRadius: BorderRadius.circular(12),
                              border: Border.all(color: optionBorder, width: isSelected || _isAnswerSubmitted ? 2.0 : 1.0),
                            ),
                            child: Row(
                              children: [
                                Icon(
                                  _isAnswerSubmitted && opt.toLowerCase() == exercise.correctAnswer.toLowerCase()
                                      ? Icons.check_circle_rounded
                                      : (_isAnswerSubmitted && isSelected && !_isCorrect
                                          ? Icons.cancel_rounded
                                          : (isSelected ? Icons.radio_button_checked : Icons.radio_button_off)),
                                  color: _isAnswerSubmitted && opt.toLowerCase() == exercise.correctAnswer.toLowerCase()
                                      ? AppColors.success
                                      : (_isAnswerSubmitted && isSelected && !_isCorrect
                                          ? AppColors.danger
            ...
            if (!_isAnswerSubmitted)
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: (_selectedOption != null || _fillBlankController.text.trim().isNotEmpty)
                      ? _submitAnswer
                      : null,
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppColors.primary,
            ...
            if (_isAnswerSubmitted) ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    _currentIndex < _exercises.length - 1 ? 'Next Question' : 'View Session Results',
  ...
  Widget _buildRuleExplanationCard(GrammarRule rule, ThemeData theme, bool isDark) {
    return Container(
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          ...
          if (rule.examples.isNotEmpty) ...[
            const SizedBox(height: 8),
            const Text('Example:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11, color: Colors.grey)),
            Text(
              '• ${rule.examples.first}',
