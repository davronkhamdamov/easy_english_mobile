...
class _GrammarRoadmapScreenState extends State<GrammarRoadmapScreen> {
  ...
  double get _overallMastery {
    ...
    final total = _topics.fold<double>(
      0.0,
      (sum, t) => sum + t.masteryPercentage,
    );
    return total / _topics.length;
  }

  GrammarTopic? get _weakestTopic {
    final active = _topics
        .where((t) => t.status != GrammarTopicStatus.locked)
        .toList();
  ...
  Widget _buildWeakTopicBanner(
    ThemeData theme,
    bool isDark,
    GrammarTopic weakTopic,
  ) {
    return Container(
      decoration: BoxDecoration(
  ...
  void _onTopicTapped(GrammarTopic topic) {
    if (topic.status == GrammarTopicStatus.locked) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Complete prerequisite topics to unlock "${topic.title}".',
          ),
    ...
    if (result != null) {
      setState(() {
        ...
        if (idx != -1) {
          ...
          if (updatedStatus == GrammarTopicStatus.completed &&
              idx + 1 < _topics.length) {
            if (_topics[idx + 1].status == GrammarTopicStatus.locked) {
              _topics[idx + 1] = _topics[idx + 1].copyWith(
  ...
  void _navigateToMistakesScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const GrammarMistakesScreen()),
    );
  }
