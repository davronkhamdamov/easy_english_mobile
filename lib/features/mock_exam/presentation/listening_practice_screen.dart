...
class _ListeningPracticeScreenState extends State<ListeningPracticeScreen> {
    ...
    if (_isPlaying) {
      _audioTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (_audioProgressSeconds < _totalAudioDurationSeconds) {
          setState(() {
            _audioProgressSeconds = (_audioProgressSeconds + (1.0 * _playbackSpeed))
                  ...
                  Row(
                    children: [
                      const CircleAvatar(
                        ...
                        child: Icon(Icons.multitrack_audio_rounded, color: AppColors.primary, size: 20),
                      ),
                      ...
                      PopupMenuButton<double>(
                        ...
                        itemBuilder: (ctx) => [
                          const PopupMenuItem(value: 0.8, child: Text('0.8x Speed')),
                          const PopupMenuItem(value: 1.0, child: Text('1.0x Speed (Normal)')),
                  ...
                  Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () {
                          setState(() {
                            _audioProgressSeconds = (_audioProgressSeconds - 10).clamp(0.0, _totalAudioDurationSeconds);
                          });
                        },
                      ),
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    leading: const Icon(Icons.subtitles_outlined, color: AppColors.secondary),
                    title: const Text(
                      'Audio Transcript',
                      style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14),
                    ),
                    subtitle: Text(
                      _showTranscript ? 'Tap to hide transcript' : 'Tap to reveal transcript text',
                      style: const TextStyle(fontSize: 11),
                    ),
                    trailing: Icon(_showTranscript ? Icons.expand_less : Icons.expand_more),
                    onTap: () {
                      setState(() {
          ...
          const SizedBox(height: 24),
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildQuestionCard(context, q, index, isDark, textPrimary);
              },
            ),
            const SizedBox(height: 24),
  ...
  Widget _buildListeningInput(MockQuestion q, bool isDark) {
    switch (q.questionType) {
      ...
      case QuestionType.sentenceCompletion:
        final controller = TextEditingController(text: _userAnswers[q.id] ?? '');
  ...
  void _submitSection({bool autoSubmitted = false}) {
    ...
    final overallBand = IeltsBandConverter.calculateOverallBand([listeningBand, 6.5]);
