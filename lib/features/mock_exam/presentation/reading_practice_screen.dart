...
import '../domain/mock_exam_model.dart';
...
class _ReadingPracticeScreenState extends State<ReadingPracticeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final currentPassage = passages.isNotEmpty ? passages[_activePassageIndex] : null;
                        ...
                        child: currentQuestion != null
                            ? Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          setState(() {
                                            if (_flaggedQuestions.contains(currentQuestion.id)) {
                                              _flaggedQuestions.remove(currentQuestion.id);
                                            } else {
                                              _flaggedQuestions.add(currentQuestion.id);
                                            }
                                          });
                                        },
                                      ),
                                  ...
                                  Expanded(
                                    child: SingleChildScrollView(
                                      child: _buildQuestionInputWidget(currentQuestion, isDark),
                                    ),
                                  ),
                                ],
                              )
            ...
            child: Row(
              children: [
                ...
                ElevatedButton(
                  ...
                  child: const Text('Submit Section', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
          ),
  ...
  Widget _buildQuestionInputWidget(MockQuestion question, bool isDark) {
    switch (question.questionType) {
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.bold,
                          color: isSelected ? optColor : (isDark ? AppColors.darkTextPrimary : AppColors.lightTextPrimary),
                        ),
                      ),
                      if (isSelected)
                        Icon(Icons.check_circle_rounded, color: optColor, size: 20),
                    ],
                  ),
                ),
              ),
      ...
      case QuestionType.sentenceCompletion:
        final textController = TextEditingController(text: _userAnswers[question.id] ?? '');
            ...
            children: [
              ...
              GridView.builder(
                ...
                itemBuilder: (context, idx) {
                  ...
                  final isAnswered = _userAnswers.containsKey(q.id) && _userAnswers[q.id]!.isNotEmpty;
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text(
                          '${idx + 1}',
                          style: TextStyle(
                            color: isAnswered || isFlagged ? Colors.white : Colors.black,
                            fontWeight: FontWeight.bold,
                          ),
                        ),
                      ),
  ...
  void _submitSection({bool autoSubmitted = false}) {
    ...
    final overallBand = IeltsBandConverter.calculateOverallBand([readingBand, 6.5]);
